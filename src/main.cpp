...
unsigned int nStakeMinAge = 6 * 60 * 60; // 24 hours
...
const string strMessageMagic = "Bitstake Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    else if (pindexBest->nHeight+1 <= 11520)
    {
      nSubsidy = 50 * COIN;
      return nSubsidy + nFees;
    }
    
    else if (pindexBest->nHeight+1 <= 12960)
    {
      nSubsidy = 30 * COIN;
      return nSubsidy + nFees;
    }
    
    else if (pindexBest->nHeight+1 <= 14400)
    {
      nSubsidy = 25 * COIN;
      return nSubsidy + nFees;
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Bitstake", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Fri, 09 Jan 2015 14:09:01 GMT";
        CTransaction txNew;
        txNew.nTime = 1420812541;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1420812541;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 4408870;
		if(fTestNet)
        {
            block.nNonce   = 4408870;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xf5187c8b824a81fd6ba7ec3037f1121c45010f8f782ba40d5e24eb8c56873f9e"));
        block.print();
    ...
    if (strCommand == "version")
    {
        ...
        if (!pfrom->fClient && !pfrom->fOneShot &&
            ...
            (pfrom->nVersion < NOBLKS_VERSION_START ||
             pfrom->nVersion >= NOBLKS_VERSION_END || pfrom->nVersion != NOBLKS_VERSION_BAD) &&
